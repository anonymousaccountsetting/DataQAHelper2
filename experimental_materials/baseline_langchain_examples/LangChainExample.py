...
This is a dataset about how the transaction date, the house age, the distance to the nearest MRT station, and the number of convenience stores affect the house price per unit area. Please use the transaction date, the house age, the distance to the nearest MRT station, and the number of convenience stores as independent variables and the house price per unit area as the dependent variable and use a linear regression model to answer the following questions.
""".strip()
...
Y_COLUMN = "Y house price of unit area"
...
# Explicitly configured model for this experiment
CONFIGURED_MODEL_NAME = "linear_regression"

# This constrains the candidate models exposed to the agent
ALLOWED_MODELS = {
    "regression": [
        "linear_regression",
...
def run_regression_model(df: pd.DataFrame, x_columns: List[str], y_column: str, model_name: str) -> Dict[str, Any]:
    ...
    if model_name == "linear_regression":
        ...
        raw_coefficients = {col: float(coef) for col, coef in zip(x_columns, raw_model.coef_)}
        standardized_coefficients = {col: float(coef) for col, coef in zip(x_columns, std_model.coef_)}
...
def run_binary_model(df: pd.DataFrame, x_columns: List[str], y_column: str, model_name: str) -> Dict[str, Any]:
    X, y = get_xy(df, x_columns, y_column)
    ...
    if model_name == "logistic_regression":
        ...
        raw_coefficients = {col: float(coef) for col, coef in zip(x_columns, model.coef_[0])}
...
def run_multiclass_model(df: pd.DataFrame, x_columns: List[str], y_column: str, model_name: str) -> Dict[str, Any]:
    X, y = get_xy(df, x_columns, y_column)
